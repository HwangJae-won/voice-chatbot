...
from util.utils import parse_args, create_logger
...
info(f'Kernel_size : {args.kernel_size}')
info(f'save_path : {save_dir}')
info(f'checkpoint_path : {args.checkpoint}')
info(f'learning_rate : {args.lr}')
info(f'prenet dropout : {prenet_dropout}')
...
def train(args):
    ...
    model = Tacotron(K=args.kernel_size, conv_dim=[128, 128]).cuda()
